...
beep = 1 #set to 1 if you want to get a pling when the script has finished
...
for(permut in permut_vec){
  ...
  for (var in vars) {
    ...
    result_df_spec <- data.frame(Feature =   var, 
    ...
    result_df <- result_df  %>% full_join(result_df_spec, by = c("Feature", "Destimate", "Pval1", "Pval0","tree_tips", "ones", "zeroes", "permut", "iteration"))
    
  }
}
}

feature_order <- c("singleton_outlier" ,"singleton_middle" ,  "singleton_random", "sisters_a"    ,     "sisters_b",         "sisters_c" ,    "two_random",     
           "triplets_a"      ,  "triplets_b"   ,    "triplets_c"   ,      "three_random" , "quadruplets_a"   ,  "quadruplets_b"   , "quadruplets_c"  ,  "four_random" , "cluster"   ,             
           "cluster_random" )
