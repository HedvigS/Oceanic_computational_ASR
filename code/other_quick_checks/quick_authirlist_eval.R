...
values <- readr::read_csv(values_csv_fn, col_types = cols(.default = "c")) %>% 
  mutate(Coders = str_split(Coders, ";")) %>% 
  unnest(Coders) %>% 
  group_by(Coders) %>% 
  summarise(n= n())
...
author_list <- readxl::read_excel("Grambank Release paper authors.xlsx") %>% 
  rename(Coders = "abbreviations") %>% 
  dplyr::select(Coders) %>% 
  mutate("in_list" = "yes")

values %>%
  full_join(author_list) %>% 
#  filter(is.na(in_list)) %>% 
  arrange(-n) %>% View() 
