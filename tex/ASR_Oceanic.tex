...
Given the basic assumption that branch lengths matter, that languages that are spoken at a similar time in history have a similar distance to the shared ancestral community's language (the root), we should choose methods that take that into account. If conventional HL-ASR does take into account branch lengths, it is often under-specified. Furthermore, it is desirable in science to be as consistent and explicit as possible. This enables others to interrogate the research and replication. 
