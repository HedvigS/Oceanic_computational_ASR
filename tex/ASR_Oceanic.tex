...
CM is primarily concerned with the reconstruction of sound inventories and vocabulary,  its application to other parts of language such as abstract grammatical patterns is less common and more controversial.  
...
The Comparative Method is typically applied to sound changes and the lexicon, but it can also be applied to structure. \citet[17-22]{clark1973aspects} wrote about Proto-Polynesian syntax for example. He outlines three important principles for reconstruction that are relevant to lexion, sounds and grammar:
...
The Comparative Method procedure that has been outlined in this section can and has been applied successfully to sounds and words --- be they lexical or grammatical.  The application of this approach to abstract structural features is more controversial. One of the concerns is whether the underlying data represents genuine correspondences (for example by satisfying the Double Cognacy Condition suggested by \citet{walkden_2013}). As a consequence of that, it can be said that not all reconstruction in historical linguistics is based on the Comparative Method in a strict sense. . Some of the work on reconstruction in historical linguistics cited in this paper can be said to represent \emph{traditional approaches}, but perhaps not necessarily the \emph{Comparative Method} in a strict sense.
